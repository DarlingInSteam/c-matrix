...
void output_static_arr(int arr[][NMAX], int arr_size_first, int arr_size_second);
void matrix_sort(int **arr, int arr_size_first, int arr_size_second);
...
int main() {
    ...
    switch (select_arr) {
        ...
        case 2:
            ...
            matrix_sort(dynamic_arr, arr_size_first, arr_size_second);
...
void matrix_sort(int **arr, int arr_size_first, int arr_size_second) {
    for (int i = 0; i < arr_size_first; i++) {
        for (int j = 0; j < arr_size_second; j++) {
            if (sum(arr, j, arr_size_second) > sum(arr, i, arr_size_second)) swap(arr, i, j);
        }
    }
}
